...
The New Session verification surface is **Implemented**, **Tested** by its deterministic suite, and both **Harness-runnable** and **Entrypoint-runnable** through `pnpm --filter @skyturn/desktop run acceptance:new-session-ui`. The real acceptance initializes a temporary Git project, drives the Electron UI, waits for Hermes and downstream execution evidence, restarts Electron, and compares the reopened UI with the SQLite projection. Its authorization and CDP control plane is bounded separately from the agent-run watchdog. On failure, it first captures a live read-only UI/workspace diagnostic, then requests graceful Electron shutdown: workflow and AgentBridge start admission close, explicit and implicit start tasks drain, owned agent process trees are reaped, terminal reconciliation drains, and only then does SQLite close. The external collector may read SQLite, private-run, Git, and artifact facts only after that shutdown succeeds; shutdown failure or post-close nonterminal evidence fails closed, and the workspace mirror remains diagnostic rather than authoritative. PTY is disabled unless explicitly enabled. Because the real path requires local Hermes/Codex credentials, compatible CLI behavior, and Electron native ABI setup, it remains **Experimental**, not **Supported**.

`pnpm --filter @skyturn/desktop run acceptance:failure-repair-regression` drives the ordinary Electron **Open Project** and **New Session** path with real Hermes planning and a real Electron-scheduled Codex run; it does not seed terminal workflow facts. A successful run exits `0` with bounded `ok=true` evidence, proves the initial failed `RunEvidence`, submits Repair through the production UI, observes the automatic real-Codex Regression handoff, validates all run and checkpoint identities, restores a clean Git worktree, reopens and reprojects the SQLite store in a separate process, then injects a stale workspace snapshot and proves the restarted renderer DOM matches the authoritative canvas before any explicit projection read. The entrypoint requires current desktop build artifacts, the matching Electron native ABI, and local Hermes/Codex credentials; it remains Experimental (`experimental-run`), not Supported (`supported-run`). The isolated `userData` directory is removed after success by default; set `SKYTURN_FAILURE_REPAIR_CLEANUP=1` to also remove the disposable project.
