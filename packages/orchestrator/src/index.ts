...
export function buildHermesWorkflowPrompt(input: HermesWorkflowPromptInput): string {
  return [
    ...
    "Schema: {\"intentId\":\"string\",\"sessionId\":\"string\",\"operations\":[{\"type\":\"AnalyzeRequirement|DiscoverProject|ProposeLanes|SplitLane|JoinLanes|StartImplementation|RequestValidation|RequestReview|RequestUserDecision|ReplanFromEvidence|Commit|DeclareEdge\"}]}",
    ...
    "SplitLane MUST include sourceLaneId and lanes; JoinLanes MUST include joinLaneId and upstreamLaneIds.",
    "StartImplementation, RequestValidation, and Commit MUST each include laneId.",
    "RequestReview MUST be {\"type\":\"RequestReview\",\"laneId\":\"the review lane id\"}; never use targetLaneId for RequestReview.",
    ...
    "Use semanticSubtype for narrower terms such as browser_validation, regression_check, or frontend_implementation.",
    "RequestUserDecision MUST include decisionId, prompt, options, reason, and optional targetLaneId or targetSegmentId.",
    "ReplanFromEvidence MUST include laneId and evidenceId; DeclareEdge MUST include sourceLaneId and targetLaneId.",
    "Use agentKind codex for implementation, command validation, and browser screenshot lanes because those lanes run local commands.",
    ...
    "Allowed operations: AnalyzeRequirement, DiscoverProject, ProposeLanes, SplitLane, JoinLanes, StartImplementation, RequestValidation, RequestReview, RequestUserDecision, ReplanFromEvidence, Commit, DeclareEdge.",
    ...
    "Hermes owns unprivileged lane granularity; SkyTurn validates and schedules the DAG lanes and edges.",
    "Do not rely on SkyTurn policy-pack fallback for unprivileged decomposition; the privileged commit exception above is required.",
