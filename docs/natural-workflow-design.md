...
当 Hermes 发现早期设计可能有重大问题时，SkyTurn 不允许它直接重写历史。系统要生成用户决策节点，由用户选择回溯、继续当前方案，或开一条隔离工作树进行双轨验证。
...
`CanvasSession` 已经保存 `hermesPlannerSessionId` 和 `plannerNodeId`，可以表达“一张画布一个逻辑 Hermes 规划身份”。相关类型在 `packages/project-core/src/index.ts`。

`workflow-kernel` 已经有 `WorkflowIntent`、`SplitLane`、`JoinLanes`、`RequestUserDecision`、`ReplanFromEvidence`，也有事件投影、门禁校验和可运行任务调度。

`ui-canvas` 目前通过 `RunEvent` 更新节点状态，并在 Hermes 输出合法 `WorkflowIntent` 后生成画布节点和边。

`agent-bridge` 已经能把 Codex 和 Hermes 的输出、退出码、失败原因、证据转成运行事件。节点完成状态已经由 `RunEvidence` 推导。

`persistence` 已经有 SQLite 工作流事件表和检查点表的雏形，但主 UI 路径还没有完全把 SQLite 作为唯一事实源。

`git-worktree` 目前只有接口和模拟实现。真实工作树创建、比较、采纳、清理还没有接入。
...
`packages/persistence/src/workflowStore.ts` 已经有 `workflow_events`、`workflow_checkpoints`、`workflow_segments` 等表。事件追加也已经有顺序号和幂等键。

但当前主路径仍然偏向渲染进程的内存状态。`packages/ui-canvas/src/workflowRuntime.ts` 会从当前 `CanvasSession` 临时重建投影，而不是从 SQLite 事件流回放。

Hermes 提示词只收到会话、规划身份、当前节点和现有节点摘要，还没有收到事件账本摘要。
...
`workflow-kernel` 已经定义 `ReplanFromEvidence`，但当前它只会写 `workflow.replan.requested`，不会自动声明修复节点和验证节点。

`ui-canvas` 有 `retryCanvasNode`，但这是原节点换新 `runId` 后重跑。这个行为不符合显式修复的产品目标。

调度器目前只认为依赖节点 `completed` 才能运行。修复节点如果依赖失败节点，会被永久卡住。
...
`workflow-kernel` 已经有 `RequestUserDecision` 操作，也有 `workflow.user_decision.requested` 事件。

缺口是 `RequestUserDecision` ABI 不够完整，没有稳定 `decisionId`、`reason`、目标对象，也没有 `answered` 事件。投影也没有把决策请求变成真正的等待状态。`waiting_input` 状态已存在，但没有在决策流程中用起来。
...
当用户选择“双轨并行”时，SkyTurn 要在本机创建隔离的 git 工作树，让两条方案在不同目录运行。两条方案可以同时执行、分别验证、分别收集变更和证据。

最终用户可以选择采纳其中一条。未采纳的工作树不能立即粗暴删除，必须先确认没有运行中的任务，再安全清理。

### 现状

`CanvasNode` 已经有 `worktree` 元数据。`startBridgeRun` 已经会把 `worktreePath` 传给 Electron main 和 `agent-bridge`。

但 Flow Kernel 投影出的节点默认 `worktree.path = "."`，实际运行会落回项目根目录。真实双轨会踩同一个工作区。

`packages/git-worktree/src/index.ts` 目前只有接口和模拟实现。Electron main 还没有创建、比较、采纳、清理工作树的 IPC。

UI 调度当前把并行度固定为 1。双轨不会自然并行。

### 落地方案

新增仅 Node 侧运行的真实工作树服务。渲染进程只能发请求，不能执行 git。真实实现放在 Electron main 或 Node-only 子路径，例如 `@skyturn/git-worktree/node`；`@skyturn/git-worktree` 根入口继续只导出 contracts、类型和 mock。

工作树创建流程：

1. Electron main 校验项目根目录是已打开项目。
2. 校验 base commit 是有效提交。
3. 生成 SkyTurn 管理路径，例如 `<project>.worktrees/session-<id>-variant-<id>/`。
4. 写入 `workflow.worktree.create_requested`。
5. 使用 `git worktree add -b <branch> <path> <baseCommit>` 创建工作树。
6. 对账 `git worktree list --porcelain`、`.git` 文件、gitdir、branch 和 HEAD。
7. 成功后写入 `workflow.worktree.created`。
8. 后续变体任务通道绑定这个真实路径。

如果第 5 步失败，写入 `workflow.worktree.create_failed`。如果进程在第 5 步之后、第 7 步之前崩溃，重启后必须通过 `create_requested` 和磁盘上的 worktree 对账，补写 `created` 或进入需要人工确认的 `orphaned` 状态。
...
并行调度要理解隔离边界。同一文件在不同工作树里修改可以并行；同一工作树内仍按文件范围冲突保守调度。
...
- 近期只采纳已经提交的变体分支，不直接采纳未提交工作区。
- 采纳前确认 `worktreeId`、`gitdir`、`branchName`、`baseCommit` 和 `headCommit` 仍然匹配事件记录。
- 采纳前做试运行合并或 cherry-pick 检查。
- 写入 `workflow.variant.adopt_requested`，再执行 git 操作。
- 成功后写 `workflow.variant.adopted`。
- 失败后写 `workflow.variant.adopt_failed`，并执行 `git merge --abort` 或 `git cherry-pick --abort`。
- 未采纳的变体写 `workflow.variant.rejected`，等待清理。

清理流程：

- 确认没有运行中的任务。
- 用 `git worktree list --porcelain` 确认目标路径确实是 git 工作树。
- 确认 `worktreeId`、`realPath`、`gitdir`、`branchName` 和事件记录一致。
...
`realpath` 包含性检查不够。采纳和清理必须绑定 `worktreeId -> repoRoot -> gitdir -> branchName -> baseCommit -> headCommit`，不能只凭路径属于管理目录就执行。
...
清理另一个变体时，确认路径属于 SkyTurn 管理范围，且 `worktreeId`、`gitdir`、branch 和事件记录一致，再使用 `git worktree remove`。
...
画布已经使用 `@xyflow/react`。节点和边由 `CanvasSession.nodes`、`CanvasSession.edges` 渲染。

`motion.ts` 已经集中管理节点入场、运行、完成、失败和边流动。GSAP 只做视觉状态，不控制图坐标。

`mergeRunEventsIntoWorkspace` 是运行事件进入 UI 的主入口。Hermes 输出合法 `WorkflowIntent` 后，`applyWorkflowIntentProjection` 会把任务通道和边映射成画布节点。

缺口是节点短句仍然偏静态，很多时候只显示固定文案。SQLite `workflow:event` 也还没有被 UI 作为主事件源订阅。
...
节点详情里的变更必须来自真实 git 数据。双轨裁决也必须比较真实证据，不能让 Hermes 用自然语言宣布哪个方案更好。

SkyTurn 可以让 Hermes 总结证据，但胜负依据必须来自测试、构建、评审、截图、性能测试和 git 变更差异。

### 现状

`Changeset` 类型已经允许 `source: "git"`，但当前 `ChangesTab` 仍然用模拟服务。

`agent-bridge` 已经能记录测试、构建、退出码和 artifact。`workflow_segments` 也能保存运行工作树路径。

### 落地方案

实现真实 `ChangesetService`。它必须在 Electron main 或 Node-only 子路径运行，渲染进程只能通过 IPC 获取摘要：

- 读取 `git status --porcelain=v1`
- 读取 `git diff --stat`
- 读取受控大小的 `git diff`
- 返回变更文件、增删统计、diff 预览和来源
...
没有数据的指标显示为未知，不让 Hermes 补故事。

### 验收

在真实 git 仓库中修改文件后，`Changes` 页签显示 `source: "git"` 的变更摘要。
...
阶段一不做双轨 worktree、真实 changeset 裁决 UI、自动修复链。先把唯一事实源打牢。
...
- 仅 Node 侧运行的真实工作树服务。
- 创建、比较、采纳、清理 IPC。
- worktree create/adopt/clean 状态机和崩溃恢复对账。
- 变体节点绑定绝对工作树路径。
- 同一文件在不同工作树里允许并行。
...
- 工作树路径被严格校验。
- worktree 身份绑定到 `worktreeId`、`gitdir`、branch、base commit 和 head commit。
- 采纳前有试运行检查。
...
新增工作流会话、ledger、Node 侧投影、调度、决策回答、工作树创建、比较、采纳、清理 IPC。`preload` 只暴露窄接口，不暴露任意 git、shell 或 filesystem 能力。
...
工作树、合并、清理都是安全敏感操作。所有操作必须由 Electron main 执行，使用最小权限、路径校验、身份绑定、试运行检查、状态机事件和回滚步骤。
...
采纳变体时用 merge 还是 cherry-pick。近期只做试运行检查后采纳已经提交的候选分支。具体策略要根据目标项目的分支模型决定。
