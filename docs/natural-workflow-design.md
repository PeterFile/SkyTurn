...
但当前产品语义还没有把“当前分支”和“新 worktree”做成会话级选择。Flow Kernel 投影出的节点默认 `worktree.path = "."`，实际运行会落回项目根目录。真实隔离 worktree 仍然没有形成用户可选的会话入口。
...
Codex TUI 的参考做法是：patch 和文件变更作为结构化数据进入 TUI，再渲染摘要。关键参考点包括 `PatchApplyBeginEvent`、`PatchApplyUpdatedEvent`、`PatchApplyEndEvent`、`TurnDiffEvent`、TUI 的 `FileChange` 模型，以及 `PatchHistoryCell` 对 `FileChange` 的摘要渲染。SkyTurn 要采用这个来源模型，而不是解析 assistant 文本。
