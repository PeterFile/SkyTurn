...
这个能力仍是实验路径，不是完整产品闭环。Hermes/Codex 适配器仍标为 `experimental-run`；New worktree 已有 UI/类型/Node 侧实现基础，但桌面 create/adopt/clean IPC 目前只记录 requested 事件；用户决策节点模型和 IPC 存在，但当前 UI 回答路径仍主要更新本地 canvas state；renderer 也保留 browser/mock fallback 和部分本地交互状态。
...
`git-worktree` 的根入口保留 browser-safe contracts/mocks；Node 子路径已经有真实 worktree service 和 git changeset reconciliation。桌面 worktree create/adopt/clean IPC 仍是 requested-event 路径，没有调用真实 side effect。
...
剩余差距是 UI 接线：当前节点弹窗里的回答动作仍主要更新本地 canvas state，没有稳定地把用户选择写入 SQLite 后再由投影恢复。
...
`startBridgeRun` 会按 session target 决定 worktree path：Current branch 落到项目根目录；New worktree 要求节点上有绝对 worktree path，否则不会启动。

`packages/git-worktree/src/node.ts` 已经有真实 `NodeGitWorktreeService`、比较、采纳、清理和恢复逻辑。但 Electron main 的 `workflow:worktree:create/adopt/clean` 目前只写 requested 事件；New worktree session 还不会自动创建并绑定真实隔离路径。

UI 调度当前仍把并行度固定为 1。双轨不会自然并行。
...
`Changes` 页签已经能展示 live structured changes、git reconciliation、mismatch 和空证据状态。缺口是采纳/提交/PR 还没有从这个 UI 打通，按钮仍是 disabled。
