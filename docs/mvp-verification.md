...
- Delivery actions can now create a controlled local commit, push the delivery branch, and create a pull request through explicit UI/IPC/backend paths. PR creation does not complete the task: `workflow.delivery.pushed` and `workflow.pull_request.created` currently record events only and do not complete Flow Kernel lanes. CI exact-head gating, merge, post-merge sync, and delivery cleanup remain next steps, and merge/cleanup must be later user-confirmed actions. Real GitHub disposable PR smoke is acceptance/test coverage, not default user-project behavior.
