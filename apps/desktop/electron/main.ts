...
ipcMain.handle("workflow:createSession", workflowHandler(async (projectRoot: string, input: WorkflowSessionCreateInput) => {
  assertKnownProjectRoot(projectRoot);
  const sessionId = optionalText(input?.sessionId) ?? requireText(input?.id, "workflow session id");
  ...
  broadcastWorkflowProjection(projectRoot, sessionId, projection);
  return { protocolVersion: RUN_PROTOCOL_VERSION, session, projection };
}));

ipcMain.handle("workflow:appendUserInput", workflowHandler(async (projectRoot: string, input: WorkflowAppendUserInput) => {
  assertKnownProjectRoot(projectRoot);
  const sessionId = requireText(input?.sessionId, "workflow session id");
  ...
  const event = store.appendWorkflowEvent({
    ...
    source: "renderer",
    idempotencyKey: optionalText(input.idempotencyKey) ?? null,
    payload: { sessionId, text },
    now: new Date().toISOString(),
  });
  const projection = store.materializeFlowProjection(sessionId);
  broadcastWorkflowProjection(projectRoot, sessionId, projection);
  return { protocolVersion: RUN_PROTOCOL_VERSION, event, projection };
}));

ipcMain.handle("workflow:ledger", workflowHandler(async (_eventProjectRoot: string, _eventSessionId: string) => {
  const projectRoot = requireText(_eventProjectRoot, "project root");
  const sessionId = requireText(_eventSessionId, "workflow session id");
  assertKnownProjectRoot(projectRoot);
  const store = await getWorkflowStore(projectRoot);
  return {
    protocolVersion: RUN_PROTOCOL_VERSION,
    ledger: buildWorkflowLedger(store.listEvents(sessionId)),
  };
}));

ipcMain.handle("workflow:applyIntent", workflowHandler(async (projectRoot: string, intent: { sessionId?: unknown }) => {
  assertKnownProjectRoot(projectRoot);
  if (typeof intent?.sessionId !== "string") throw workflowIpcError("INVALID_INPUT", "WorkflowIntent sessionId is required.");
  const store = await getWorkflowStore(projectRoot);
  ...
  broadcastWorkflowProjection(projectRoot, intent.sessionId, projection);
  return { protocolVersion: RUN_PROTOCOL_VERSION, result, projection };
}));

ipcMain.handle("workflow:scheduleReady", workflowHandler(async (projectRoot: string, input: unknown) => {
  assertKnownProjectRoot(projectRoot);
  const sessionId = requireText(readField(input, "sessionId"), "workflow session id");
  const allowedParallelism = positiveInteger(readField(input, "allowedParallelism")) ?? 1;
  const runningScopes = Array.isArray(readField(input, "runningScopes"))
    ? readField(input, "runningScopes") as Array<{ fileScopes: string[]; packageScopes: string[] }>
    : undefined;
  const store = await getWorkflowStore(projectRoot);
  const { scheduleReadyLanes } = await import("@skyturn/workflow-kernel");
  const projection = store.materializeFlowProjection(sessionId) as FlowProjectionLike;
  const schedule = scheduleReadyLanes as (projection: unknown, input: {
    allowedParallelism: number;
    runningScopes?: Array<{ fileScopes: string[]; packageScopes: string[] }>;
  }) => unknown[];
  const ready = schedule(projection, { allowedParallelism, runningScopes });
  return { protocolVersion: RUN_PROTOCOL_VERSION, ready };
}));

ipcMain.handle("workflow:recordRunResult", workflowHandler(async (projectRoot: string, input: WorkflowRecordRunResultInput) => {
  assertKnownProjectRoot(projectRoot);
  const sessionId = requireText(input?.sessionId, "workflow session id");
  const laneId = requireText(input?.laneId, "workflow lane id");
  const segmentId = requireText(input?.segmentId, "workflow segment id");
  const runId = requireText(input?.runId, "agent run id");
  const status = normalizeSegmentStatus(input?.status);
  ...
  const events = [
    store.appendWorkflowEvent({
      ...
      source: normalizeAgentKind(input?.agentKind),
      idempotencyKey: `segment:${segmentId}:started`,
      payload: { segment: { id: segmentId, laneId, runId, status: "running", exitCode: null } },
      now,
    }),
  ];
  if (isRecord(input?.evidence)) {
    events.push(store.appendWorkflowEvent({
      sessionId,
      kind: "workflow.evidence.recorded",
      source: normalizeAgentKind(input.agentKind),
      idempotencyKey: optionalText(input.idempotencyKey) ?? `segment:${segmentId}:evidence`,
      payload: {
        laneId,
        segmentId,
        evidence: normalizeRunEvidenceForWorkflow(segmentId, input.evidence, status, exitCode),
      },
      now,
    }));
  }
  events.push(store.appendWorkflowEvent({
    ...
    payload: { laneId, segmentId, status, exitCode },
    now,
  }));
  const projection = store.materializeFlowProjection(sessionId);
  broadcastWorkflowProjection(projectRoot, sessionId, projection);
  return { protocolVersion: RUN_PROTOCOL_VERSION, events, projection };
}));
...
ipcMain.handle("workflow:events", workflowHandler(async (projectRoot: string, sessionId: string) => {
  ...
  return {
    protocolVersion: RUN_PROTOCOL_VERSION,
    events: workflowEventsOnly(store.listEvents(sessionId)),
  };
}));
